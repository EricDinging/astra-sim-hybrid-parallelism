...
void LocalMemUsageTracker::recordWrites(
    ...
    for (const auto& iter : IOinfos) {
        ...
        if (this->tensorSize.find(tensorName) == this->tensorSize.end()) {
            ...
            // Within one iteration each tensor is written once, but
            // multi-iteration replay re-runs the same trace: keep the latest
            // write instead of the old assert(false), which aborted at
            // iteration 2 with the tracker on.
