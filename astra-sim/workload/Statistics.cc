...
void Statistics::report(std::shared_ptr<spdlog::logger> logger) const {
    ...
    for (const auto& [node_id, stat] : operator_statistics) {
        if (stat.type == OperatorStatistics::OperatorType::COMM &&
            stat.network_bandwidth.has_value() && stat.comm_size.has_value()) {
            ...
            // logger->info("  Node {}: Size: {} bytes, Duration: {} ns, "
            //              "Bandwidth: {:.3f} GB/s ",
            //              node_id, stat.comm_size.value(),
            //              stat.end_time - stat.start_time, bandwidth_gbps);
        }
    }
