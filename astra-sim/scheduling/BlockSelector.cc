...
std::optional<Placement> BlockSelector::select(
    const FoldVariant& v,
    ...
    return select(v, free, dims, cm, scorer, ranker, ring_edges, scratch);
}
...
std::optional<Placement> ContiguousFirst::select(
    ...
    scan_contiguous_fits(
        v, dims, scratch.free_mask.data(), K, any_fits,
        [&](const std::vector<int>& cand, const std::array<int, 3>& pf) {
            ...
            const auto [frag, blocks] = scratch.frag_blocks(cand, cm);
...
std::optional<Placement> MinReconfig::select(
    ...
    ContiguousFirst cf;
    auto c = cf.select(v, free, dims, cm, scorer, ranker, ring_edges, scratch);
    ...
    do {  // identity orientation first, then the other axis rotations
        ...
        auto rm = cm.ocs_enabled()
                      ? scatter_assign_nested(pv, free, dims, cm, ring_edges,
                                              budget_, &scratch)
        ...
        const auto [frag, blocks] = scratch.frag_blocks(*rm, cm);
