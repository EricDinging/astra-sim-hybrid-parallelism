...
HalvingDoubling::HalvingDoubling(ComType type,
    ...
    this->log2_nodes = log2(nodes_in_ring);
    ...
    case ComType::All_Reduce:
        stream_count = 2 * log2_nodes;
        break;
    default:
        stream_count = log2_nodes;
    }
    if (type == ComType::All_Gather) {
        max_count = 0;
    } else {
        max_count = log2_nodes;
    }
    ...
    default:
        LoggerFactory::get_logger("system::collective::HalvingDoubling")
            ->critical(
                ...
                "HalvingDoubling collective algorithm #########");
        std::exit(1);
    }
...
int HalvingDoubling::get_non_zero_latency_packets() {
    return log2_nodes - 1 * parallel_reduce;
}
