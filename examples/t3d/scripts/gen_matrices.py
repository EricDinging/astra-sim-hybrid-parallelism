"""Generate torus BW/LT schedule matrices for the reconfigurable frontend.

Emits ``bandwidth_schedule.txt`` and ``latency_schedule.txt`` for a Dx x Dy x Dz
3-D torus (default 8x8x8 = 512 NPUs). Each file is a single NxN block framed by
...
def write_matrix(
    path: str, tag: str, n: int, dx: int, dy: int, dz: int, value: int
) -> None:
    """Write one NxN torus block: ``<tag> 0`` ... N rows ... ``END``."""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="t3d torus BW/LT matrix generator")
    ...
    write_matrix(bw_path, "BW", n, dx, dy, dz, args.bw)
    write_matrix(lt_path, "LT", n, dx, dy, dz, args.lt)
    print(
        f"wrote {bw_path} and {lt_path}: {dx}x{dy}x{dz} torus (N={n}),"
        f" bw={args.bw} lt={args.lt}, 6 neighbors/row"
    )
    return 0
